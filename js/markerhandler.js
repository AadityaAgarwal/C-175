...
AFRAME.registerComponent("markerhandler", {
    ...
    if (elementsArray.length > 1) {
      ...
      if (length === 3) {
        ...
        if (distance1 < 1.25 && distance2 < 1.25) {
          if (compound !== undefined) {
            var barcode_value = elementsArray[0].barcodeValue
            this.showCompound(compound)
          }
          else {
            messageText.setAttribute('visible', true)
      ...
      if (A.includes(el.element_name)) {
        ...
        for (var i of elementsArray) {
          ...
          if (C.includes(i.element_name)) {
            var count = this.countOccurances(elementsArray, el.elementName)
            if (count > 1) {
  ...
  countOccurances: function (arr, val) {
    return arr.reduce((a, v) => {
      (v.element_name == val ? (a + 1) : (a), 0);
    });
  },
});
